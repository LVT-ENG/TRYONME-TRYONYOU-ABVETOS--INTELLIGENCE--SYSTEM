...
deploy_express() {
  local ZIP_PATH="$1"
  local SCRIPT="$HOME/deploy_express.sh"
  if [ -f "$SCRIPT" ]; then
    echo "${RED}Deploying: $ZIP_PATH${NC}"
    zsh "$SCRIPT" "$ZIP_PATH"
  else
    echo "${RED}Deploy script not found: $SCRIPT${NC}"
  fi
}
...
move_zip() {
  ...
  local TARGET="$DEST/${TS}__${BASE}"

  echo "${RED}Moving: $FILE → $TARGET${NC}"
  mv "$FILE" "$TARGET"

  deploy_express "$TARGET"
}
...
echo "${RED}Scanning directories...${NC}"
...
echo "${RED}Organization and Deploy Express complete.${NC}"
