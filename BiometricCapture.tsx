...
export default function BiometricCapture() {
  ...
  const [measurements, setMeasurements] = useState<{ shoulder: string; waist: string; height: string } | null>(null);
  ...
  const predictWebcam = () => {
    ...
    const renderLoop = () => {
      if (!isScanning && scanComplete) return; // Stop loop if complete
      ...
      if (video.currentTime !== lastVideoTime) {
        ...
        landmarker.detectForVideo(video, startTimeMs, (result) => {
          ...
          if (result.landmarks) {
            ...
            if (result.landmarks.length > 0 && !scanComplete) {
              // Calculate real-time measurements (approximated from pixel distance relative to frame height)
              ...
              const torsoHeightPx = dist(shoulderLeft, hipLeft);
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-between p-8 pointer-events-none">
          ...
          <div className="w-full flex justify-between items-start">
            <div className="flex flex-col">
              <h2 className="text-white font-serif text-2xl tracking-widest uppercase">Biometric Scan</h2>
              <p className="text-white/60 font-sans text-xs tracking-widest">GALERIES LAFAYETTE • PARIS</p>
            </div>
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isScanning ? "bg-red-500 animate-pulse" : "bg-white/20"}`} />
              <span className="text-white/60 font-mono text-xs">SYSTEM {isScanning ? "ACTIVE" : "STANDBY"}</span>
            </div>
          </div>
          ...
          {cameraError && (
            <div className="flex flex-col items-center gap-4 text-red-500">
              ...
              <p className="font-mono text-sm uppercase tracking-widest">{cameraError}</p>
              <Button 
            ...
            <div className="w-full max-w-md flex flex-col gap-4">
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div className="bg-black/50 border border-white/10 p-2 text-center backdrop-blur-sm">
                  <div className="text-[10px] text-white/50 uppercase tracking-wider">Shoulder</div>
                  <div className="text-red-500 font-mono text-lg">{measurements?.shoulder || "--"}</div>
                </div>
                <div className="bg-black/50 border border-white/10 p-2 text-center backdrop-blur-sm">
                  <div className="text-[10px] text-white/50 uppercase tracking-wider">Waist</div>
                  <div className="text-red-500 font-mono text-lg">{measurements?.waist || "--"}</div>
                </div>
                <div className="bg-black/50 border border-white/10 p-2 text-center backdrop-blur-sm">
                  <div className="text-[10px] text-white/50 uppercase tracking-wider">Height</div>
                  <div className="text-red-500 font-mono text-lg">{measurements?.height || "--"}</div>
                </div>
              </div>
          ...
          {scanComplete && (
            ...
            >
              ...
              <h3 className="text-3xl font-serif text-white tracking-widest">Scan Complete</h3>
              <p className="text-white/60 font-sans max-w-md text-center">
                Your biometric profile has been securely generated. Proceed to your curated selection.
              </p>
              <Button 
