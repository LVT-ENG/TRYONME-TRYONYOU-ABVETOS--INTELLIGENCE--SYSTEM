...
export default function MagicMirror() {
  ...
  const [lang, setLang] = useState('fr'); // Default to French for Lafayette

  const handleScan = async () => {
    setLoading(true);
    try {
      // Mock biometrics for the demo
      const payload = {
  ...
  return (
    <div style={{backgroundColor: '#121212', color: '#C5A46D', minHeight: '100vh', padding: '40px', textAlign: 'center', fontFamily: 'sans-serif'}}>
      ...
      <div style={{border: '2px solid #C5A46D', margin: '20px auto', width: '80%', minHeight: '500px', borderRadius: '15px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px'}}>

        {!result && !loading && (
           <p style={{fontSize: '1.2rem'}}>📸 Escaneando fotos de /assets/vision/... <br/> 👗 Conectando con Base de Datos Lafayette...</p>
        )}
      ...
      {!result && (
        <button
          onClick={handleScan}
          disabled={loading}
          style={{padding: '15px 30px', backgroundColor: '#C5A46D', border: 'none', color: 'black', fontWeight: 'bold', cursor: 'pointer', opacity: loading ? 0.7 : 1}}
        >
          {loading ? 'ANALYSING...' : 'PROBAR LOOK DE LAFAYETTE'}
        </button>
      )}
    </div>
  );
